...
export default function Navbar() {
  ...
  const isActive = (paths) =>
    paths.some((p) =>
      typeof p === "string" ? location.pathname.startsWith(p) : p.test(location.pathname)
    );
  ...
  return (
    <>
      ...
      <nav
        ...
        style={{ background: "#1565c0", position: "sticky", top: 0, zIndex: 1000, padding: "0 1rem", minHeight: 48 }}
      >
        <div className="container-fluid px-0 gap-1">

          <Link className="navbar-brand fw-bold me-4 mb-0" to="/dashboard" onClick={close}
          ...
          <div className="d-flex align-items-center gap-1 flex-grow-1">
            ...
            {/* Reservas — sin "Calendario de reservas" (duplicado con Rack) */}
            <NavDropdown
              label="Reservas"
            ...
            {/* Facturacion */}
            <NavLink to="/facturacion" label="Facturacion"
              active={isActive(["/facturacion"])} onClick={close} />

            {/* Reportes */}
            <NavLink to="/reportes" label="Reportes"
              active={isActive(["/reportes"])} onClick={close} />

            {/* Administracion — solo admin, ahora unificado */}
            {isAdmin && (
              <NavDropdown
          ...
          <div className="d-flex align-items-center gap-2 ms-auto">
            <span className="text-white-50 d-none d-md-inline" style={{ fontSize: 13 }}>
            ...
            <Notificaciones />
            <button className="btn btn-sm btn-outline-light" onClick={handleLogout} style={{ fontSize: 13 }}>
              Cerrar sesion
            </button>
          </div>
...
function NavDropdown({ label, open, active, onToggle, children }) {
  return (
    <div className="position-relative">
      <button className="btn btn-sm px-2 py-1" onClick={onToggle} style={{
      ...
      {open && (
        <div className="position-absolute shadow rounded py-1" style={{
          top: "calc(100% + 2px)", left: 0, minWidth: 210,
          background: "#fff", zIndex: 1001, border: "1px solid rgba(0,0,0,0.1)",
        }}>
          {children}
        </div>
      )}
...
function DropItem({ to, label, onClose }) {
  ...
  return (
    <Link to={to} onClick={onClose}
      ...
      style={{ color: active ? "#1565c0" : "#1a1a2e", background: active ? "#e3f2fd" : "transparent",
...
function NavLink({ to, label, active, onClick }) {
  return (
    <Link to={to} onClick={onClick} style={{
      ...
      background: active ? "rgba(255,255,255,0.15)" : "transparent",
      border: "none", fontSize: 14, fontWeight: active ? 600 : 400,
      borderRadius: 4, textDecoration: "none", padding: "4px 8px",
